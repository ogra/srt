...
extern "C" void
TestLogHandler(void* opaque, int level, const char* file, int line, const char* area, const char* message);
...
int parse_args(LiveTransmitConfig& cfg, int argc, char** argv)
{
    const OptionName o_timeout = {"t", "to", "timeout"}, o_timeout_mode = {"tm", "timeout-mode"},
                     ...
                     o_statspf = {"pf", "statspf"}, o_statsfull = {"f", "fullstats"}, o_loglevel = {"ll", "loglevel"},
                     o_logfa = {"lfa", "logfa"}, o_log_internal = {"loginternal"}, o_logfile = {"logfile"},
    ...
    if (print_help)
    {
        ...
        PrintOptionHelp(
            o_timeout_mode, "<mode=0>", "timeout mode (0 - since app start; 1 - like 0, but cancel on connect");
#endif
...
int main(int argc, char** argv)
{
    ...
    if (cfg.log_internal)
    {
        srt_setlogflags(0 | SRT_LOGF_DISABLE_TIME | SRT_LOGF_DISABLE_SEVERITY | SRT_LOGF_DISABLE_THREADNAME |
        ...
        while (!int_state && !timer_state)
        {
            if (!src.get())
            {
                ...
                case UriParser::SRT:
                    ...
                    {
                        cerr << "Failed to add SRT source to poll, " << src->GetSRTSocket() << endl;
                        return 1;
                    }
                    break;
                case UriParser::UDP:
                case UriParser::RTP:
                    ...
                    {
                        cerr << "Failed to add " << src->uri.proto() << " source to poll, " << src->GetSysSocket()
                             << endl;
                        return 1;
                    }
                    break;
                case UriParser::FILE:
                {
                    ...
                    {
                        cerr << "Failed to add FILE source to poll, " << src->GetSysSocket() << endl;
                        return 1;
                    }
                    break;
                }
            ...
            if (!tar.get())
            {
                ...
                case UriParser::SRT:
                    ...
                    {
                        cerr << "Failed to add SRT destination to poll, " << tar->GetSRTSocket() << endl;
                        return 1;
                    }
                    break;
                default:
                               ...
                               0) >= 0)
            {
                ...
                while (!dataqueue.empty())
                {
                    ...
                    if (!tar.get() || !tar->IsOpen())
                    {
                        lostBytes += pkt->payload.size();
                                 ...
                                 pkt->payload.data(), pkt->payload.size(), cfg.srctime ? pkt->time : 0, out_stats))
                    {
                        lostBytes += pkt->payload.size();
                    }
                    else
                ...
                if (!cfg.quiet && (lastReportedtLostBytes != lostBytes))
                {
                    ...
                    {
                        cerr << lostBytes << " bytes lost, " << wroteBytes << " bytes sent, " << receivedBytes
                             << " bytes received" << endl;
